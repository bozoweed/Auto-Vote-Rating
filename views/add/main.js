...
(function (root, factory) {
  ...
  async function checkPermissions(projects, element, backend) {
    ...
    var getDomainWithoutSubdomain = (backend.utils && backend.utils.getDomainWithoutSubdomain) || (u => { try { var h=new URL(u).hostname; var p=h.split('.'); return p.slice(-2).join('.'); } catch(e){ return u; } });
  ...
  AVRFW.createViewProvider('add', {
    ...
    onMounted: function(ctx){
      ...
      (async function initAdd(){
        ...
        // Injected backend (DI); ensure availability via framework helper
        const backend = await AVRFW.ensureBackend(ctx, { install: true });
        if (!backend) { notif.create('Backend not available', 'error'); return; }

        var db = backend.DB, settings = backend.SETTINGS || {}, allProjects = backend.allProjects || {};
        var getDomainWithoutSubdomain = backend.utils && backend.utils.getDomainWithoutSubdomain;
        ...
        E('#addProject').addEventListener('submit', async function(ev){
          ...
          try {
            ...
            var statusEl = null;
            if (!(await checkPermissions([project], statusEl, backend))) return;
        ...
        (async function boot(){
          try { backend.attachGlobalErrorHandlers && backend.attachGlobalErrorHandlers(function(err){ OptionsCore.getNotif().create(err, 'error', { error: err }); }); } catch(e){}
          
          if (!db || typeof db.get !== 'function') {
           
            try { await backend.initializeConfig({ background: false }); }
            catch (err) { console.error(err); }
            db = backend.DB;
          }

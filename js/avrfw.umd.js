    ...
    function compileNode(el, ctx, wiring) {
        ...
        if (hasAttr(el, 'each')) {
            ...
            function compileTextNode(textNode, rowCtx, rowCleanups) {
                ...
                const eff = effect(() => {
                    const out = parts.map(p =>
                        p.type === 'text' ? p.v : (p.fn(state, rowCtx, methods) ?? '')
                    ).join('');
                    textNode.nodeValue = out;
                });
                rowCleanups.push(() => cleanup(eff));
            }

            // per-render row teardowns
            const rowTeardowns = [];
            function clearRows() {
            ...
            const render = () => {
                ...
                // dispose previous row effects/listeners before DOM clear
                clearRows();
                el.textContent = '';
                ...
                arr.forEach((item, i) => {
                    ...
                    // compile everything we just appended
                    appended.forEach(n => {
                        if (n.nodeType === 1) {
                            // element
                    ...
                    rowTeardowns.push(() => {
                        rowCleanups.forEach(fn => { try { fn(); } catch { } });
                    });
                });
            };
    ...
    function createApp(opts = {}) {
        ...
        async function loadView(name, baseUrl, opt = {}) {
            ...
            await new Promise((resolve, reject) => {
                ...
                s.async = true; s.defer = false; s.src = url(jsPath);
